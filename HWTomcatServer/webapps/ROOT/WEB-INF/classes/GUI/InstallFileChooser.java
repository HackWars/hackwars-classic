...
public class InstallFileChooser extends JInternalFrame implements ActionListener,MouseListener{
	...
	public InstallFileChooser(Hacker MyHacker,String folder,Object[] directory,PortManagement MyPortManagement,int port){
		...
		this.port=port;
		MyHacker.setInstallFileChooser(this);
		...
		MyView.addFunctionCall(new RemoteFunctionCall(Hacker.INSTALL_FILE_CHOOSER,"requestdirectory",objects));
			...
			try{
				for(int i=0;i<directory.length;i++){
					if(directory[i] instanceof String){
						...
						if(type==HackerFile.BANKING_COMPILED||type==HackerFile.ATTACKING_COMPILED||type==HackerFile.FTP_COMPILED||type==HackerFile.HTTP||type==HackerFile.SHIPPING_COMPILED){
