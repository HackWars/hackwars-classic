...
public class FirewallQuantityDialog extends JInternalFrame implements ActionListener{
	...
	public FirewallQuantityDialog(FirewallBrowser firewallBrowser, Hacker MyHacker,int maxQuantity,String name,float price,int index){
		this.firewallBrowser=firewallBrowser;
	...
	public void actionPerformed(ActionEvent e){
		...
		if(e.getSource()==ok){
			firewallBrowser.sellFile(index,(int)((Integer)spinner.getValue()).intValue());
			setVisible(false);
		}
        if (e.getSource()==sellAll) {
            firewallBrowser.sellFile(index,maxQuantity);
            setVisible(false);
        }
